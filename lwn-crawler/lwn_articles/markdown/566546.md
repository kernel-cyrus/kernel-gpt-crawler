...
The Btrfs filesystem has the beginning of support for offline
     deduplication of data blocks.  A new
ioctl()
command
     (
BTRFS_IOC_FILE_EXTENT_SAME
) can be used by a user-space
...
The
reliable out-of-memory killer
     patches
have been merged.  This work should make OOM handling more
     ...
     not seen before.
The evdev input layer has gained a new
EVIOCREVOKE
ioctl()
command that revokes all access to a given file
     descriptor.  It can be used to ensure that no evil processes lurk on
     an input device across sessions.  See
this
     patch
for an example of how this functionality can be used.
...
The seqlock locking primitive has gained a new "locking reader" type.
     ...
     normal readers through.  Note that locking readers
could
share
